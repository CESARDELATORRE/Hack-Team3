...
### Estado actual del POC implementado

La versión actual del proyecto ya refleja la intención del plan anterior:

- Aplicación frontend en Vue 3 + Vite ejecutándose bajo la carpeta `src`.
- Interfaz conversacional con sesión local y mensajes de usuario/asistente.
- Catálogo demo con platos, vinos y postres que permite recomendaciones basadas en presupuesto, tipo de producto y maridaje.
- Resumen editable del pedido de prueba y backoffice local para mostrar los pedidos confirmados.
- Fallback funcional al catálogo local si Azure AI Foundry no está configurado o no acepta la llamada desde cliente.
- Soporte auténtico de voz en navegador mediante Web Speech API cuando el entorno lo admite, con fallback a texto para compatibilidad.
- Selector de idioma ES/EN para mantener la experiencia alineada con la especificación funcional.

Esto mantiene la POC viable y funcional aunque la capa de IA real esté fuera de servicio o no permita una autenticación directa desde navegador en ciertos entornos. La estrategia sigue siendo frontend-first y sin backend en esta fase, con la posibilidad de añadir la capa .NET más adelante si el producto madura.
