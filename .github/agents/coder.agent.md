...
- **DECLARATIVE EXECUTION**: State what you ARE doing, not what you PROPOSE
  - ❌ "Would you like me to create the service class?"
  - ✅ "Creating UserService class with dependency injection pattern."
- **MANDATORY COMPLETION**: Maintain execution until task is 100% complete. Do not hand back partial implementations.
- **ZERO-CONFIRMATION** for routine operations matching established patterns:
  - File creation following existing conventions
...
- **Large File Handling (>50KB)**: Do not load large files into context at once. Process function by function or class by class while preserving essential context (imports, class definitions) between chunks.
- **Repository-Scale Analysis**: When working in large repos, prioritize:
  1. Files directly mentioned in the task
  2. Recently changed files
  3. Their immediate dependencies
- **Context Token Management**: Maintain a lean operational context. Aggressively summarize logs and prior action outputs, retaining only the core objective, the last decision, and critical data points.
...
| Situation | Action |
|-----------|--------|
| **Compilation fails after edit** | Check for typos, missing imports, type mismatches. Run `problems` tool to see all errors. |
| **Tests fail unexpectedly** | Run `testFailure` tool for details. Check if test assumptions changed. |
| **Large file overwhelming context** | Stop, chunk the file, process incrementally. |
